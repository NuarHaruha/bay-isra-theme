...
function bones_custom_admin_footer() {
	echo '<span id="footer-thankyou">Copyright &copy; 2012. D Baydura Sdn. Bhd. All Rights Reserved.';
}
...
{ 
    ...
    wp_register_style( 'mdag-style', get_template_directory_uri() . '/library/css/base.css', array(), '1.0', 'all' );
    
    foreach(array(
        'bootstrap',
        'bootstrap-responsive',
        'wp-bootstrap',
        'mdag-style') as $handler){
...
function mdag_breadcrumbs() {
    global $post;
    if ((is_page() && !is_front_page()) || is_home() || is_category() || is_single()) {
        echo '<ul class="breadcrumb bg-trans">';
        echo '<li><a href="'.get_bloginfo('url').'"><i class="icon-home"></i> Home</a> <span class="divider">/</span></li>';
        ...
        if ($post_ancestors) {
            ...
            foreach ($post_ancestors as $crumb)
                echo '<li><a href="'.get_permalink($crumb).'">'.get_the_title($crumb).'</a> <span class="divider">/</span></li>';
            }
        
        if (is_category() || is_single() && !is_front_page()) {
